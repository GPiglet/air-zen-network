...
const Sticky: FC = () => {
    ...
    return (
        <div className={`bg-black fixed bottom-0 w-full py-7 z-60 border border-third border-opacity-30 ${show ? 'block' : 'hidden'}`}>
            <div className="container mx-auto flex  md:pl-16 flex-col-reverse md:flex-row">
                <div className="flex flex-col mx-16 md:ml-0 mt-5 md:mt-0">
                    <button className="w-full md:w-max font-lato text-white text-base bg-gradient-to-r from-secondary to-[#669AB4] py-2.5 px-7 rounded-md" onClick={() => { setStorage('cookieAllow', true); setShow(false) }}>
                        {t('cookie.accept')}
                    </button>
                    <div className="bg-gradient-to-r w-full p-[1px] rounded-md from-secondary to-[#669AB4] mt-3">
                        <button className=" font-lato w-full text-white text-base bg-black py-2.5 border- rounded-md" onClick={() => { setStorage('cookieAllow', true), setShow(false) }}>
                            {t('cookie.customize')}
                        </button>
