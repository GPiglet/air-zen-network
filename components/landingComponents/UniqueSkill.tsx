...
const UniqueSkill: FC = () => {
    ...
    const skillList = [
        ...
        },
        {
            ...
            description: 'Das Team und ihre Kunden:innen - damit jeder überall und jederzeit vernetzt ist, gibt es die smarten und sicheren Netzwerklösungen von AirZen.',
            list: [],
        ...
        {
            title: 'Education',
            subtitle: 'Für eine digitale Zukunft.',
            description: 'Digitaler Wandel, Homeschooling, Verwaltung - damit alles funktioniert braucht es ein komplexes WLAN mit kinderleichter Handhabe.',
            list: [],
        ...
        {
            title: 'Industrial',
            subtitle: 'Für den digitalen Fortschritt.',
            description: 'Innovative Ideen brauchen individuelle Lösungen - Auch im großen Stil. AirZen hilft beider Lösung industrieller Netzwerk-Aufgaben.',
            list: [],
            ...
            <div id='solutions' ref={containerRef} className='z-10 md:container mx-auto relative md:fixed md:left-[50%] md:translate-x-[-50%] md:flex justify-center items-center pb-[200px]'>
                <div className='mt-[200px] md:mt-[120px]'>
                    <div className=" text-center">
                        <h1 ref={el=>animSideUp.current.push(el)} className="text-title-sm">Einzigartige Technologie. </h1>
                        <p ref={el=>{animSideUp.current.push(el);animFadeIn.current.push(el)}} className="font-lato font-light tracking-widest text-white text-[22px] mt-3">Unzählige Einsatzmöglichkeiten. Finden Sie Ihre:</p>
                    </div>
                    ...
                    <div className='md:m-auto md:w-max'>
                        {
                            skillList.map((item, index) => (
                                // 
                                <div ref={el=>animSkills.current.push(el)} className='right-[-20px] sm:right-[-70px] w-full md:w-[200px] xl:w-[300px] md:inline-block align-top md:right-auto relative px-5 py-5 flex-1 unique-skill-items unique-skill-animate z-40' key={index} onClick={() => router.push(item.href)}>
