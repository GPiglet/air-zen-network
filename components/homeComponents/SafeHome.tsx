...
const SafeHome: FC<{ props?: any, ref: any }> = React.forwardRef((props: any, ref: any) => {
    ...
    return (
        <div id="secure" ref={(el) => { containerRef.current = el; if (ref) ref.current = { container: el, startAnim } }} className="container mx-auto relative md:h-screen md:fixed md:hidden md:left-[50%] md:translate-x-[-50%]">
            <svg className="absolute left-[-27%] top-[28.5%] w-[140%] sm:top-[37%]  md:top-1/2 md:translate-y-[-50%] sm:w-[120%] md:w-[100%] xl:w-[80%]   sm:left-[-10%]  md:left-[-45%] xl:left-[-32%]" viewBox="0 0 804 796" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <svg ref={refAnimRightImage} className="absolute right-[-52%] top-[58%] sm:top-[32%] md:top-[90%] md:w-[120%] xl:w-[100%]   sm:w-[140%] sm:right-[-20%] w-[204%] md:right-[-70%] xl:right-[-53%] md:translate-y-[-50%]" viewBox="0 0 907 907" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div ref={el => {if (el && refAnimContents.current.indexOf(el) == -1)refAnimContents.current.push(el)}} className=" relative md:fixed md:hidden items-center px-10 md:px-0 h-full mt-[110%] md:mt-0">
                <div className="flex flex-wrap">
                    {/* <div className="w-[20%]"></div> */}
                    <div className="w-full md:w-1/2 m-auto">
